...
function readState() {
  try {
    ...
    return {
      companyTokens: Array.isArray(parsed.companyTokens) ? parsed.companyTokens : [],
...
app.get('/oauth/callback', async (req, res) => {
  try {
    ...
    const token = await exchangeCodeForToken(code, 'Company');
    const companyId = normalizeText(token.company_id);
    if (!companyId) {
    ...
    const returnTo = normalizeText(state.returnTo);
    if (returnTo) {
      const url = new URL(returnTo);
      url.searchParams.set('connected', '1');
      url.searchParams.set('install_type', 'agency');
      url.searchParams.set('company_id', companyId);
      url.searchParams.set('bulk_discovered', String(bulkRetry.final?.discovered || 0));
      url.searchParams.set('bulk_minted', String(bulkRetry.final?.minted || 0));
      url.searchParams.set('bulk_retry_success', bulkRetry.success ? '1' : '0');
      res.redirect(url.toString());
      return;
    }
    res.json({ ok: true, companyId, bulkRetry });
  } catch (error) {
